...
async function updateAllSoftware(softwares, downloads) {
  for (let i = 0; i < softwares.length; ++i) {
    ...
    const download = getOrMakeDownloadObject(downloads, software);
    try {
      ...
      if (version == download.version) return;
...
function getOrMakeDownloadObject(downloads, software) {
  if (!downloads[software.title])
    downloads[software.title] = Object.assign({}, software);
...
async function downloadFile(url, filename) {
  ...
  await download(url, "downloads", { filename: filename });
...
function moveOldFile(oldFilePath) {
  if (oldFilePath) {
    ...
    fs.rename(oldFilePath, `downloads/old/${oldFileName}`, () => {});
  }
}
